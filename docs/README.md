...
| **What the packages must let a consumer do** — durable, checkable product intent  | [`docs/product/`](./product/README.md) — one file per requirement, with a declared state ([ADR-093](decisions/ADR-093-declare-a-requirement-state-rather-than-derive-it.md))                                          |
...
| **What the merge queue changes, and how to read a pull request it ejected**       | [`docs/tooling/merge-queue.md`](./tooling/merge-queue.md) — the `merge_group` trigger rule, what each gate diffs against inside the queue, the ruleset payload                                                        |
...
| `docs/product/requirements/*.md` | every agent + human      | durable            | what a consumer must be able to do, and whether that is true yet — one file each, no index                                                                            |

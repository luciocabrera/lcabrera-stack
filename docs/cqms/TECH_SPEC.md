...
- `apps/scan-orchestrator` needs its own Zod-validated `ANTHROPIC_API_KEY`, never logged, never returned anywhere — and, per §2.7's two-process split, `admin_system` never needs this key at all, since it never runs a scan itself.
...
## 2.7 Real-time status — a standalone `scan-orchestrator` process, WebSockets (not polling)

Per explicit follow-up decision, run/scan status pushes to the UI in real time over a WebSocket rather than the UI polling an interval. Superseded design (kept in git history, not reproduced here): a single custom `admin_system/server.ts` doing both CRUD and WebSocket duty. Revised after weighing the tradeoffs directly against `apps/admin_system` staying on stock `react-router dev`/`@react-router/serve`, unmodified — see the decision ADR for the full pros/cons; this section describes the design that decision produced.

**Why WebSockets over polling at all, unchanged from the original reasoning**: a polling loop needs a `useEffect` to drive the timer, which is an awkward fit for "zero `useEffect` for server data" — it's fetching, just on a schedule instead of on mount. A WebSocket _subscription_ is a canonical, accepted `useEffect` use case (synchronizing with an external system), so this is a guideline-compliant addition, not an exception.

### Two processes, not one

- **`apps/admin_system`** — **completely untouched**. Still `react-router dev` in dev, still `@react-router/serve` in prod, no custom `server.ts`, no Express, no Vite `configureServer` plugin. It only ever does one new thing: its `trigger-scan` action inserts `queued` run/scan rows (already true today) — it never spawns, awaits, or knows about the job that executes them.
- **`apps/scan-orchestrator`** (new) — a small, plain Node/TypeScript process with no React, no Vite, no React Router. Owns three things: (1) noticing a newly-queued scan, (2) actually running it (branching per `scanners.deterministic`, same as originally planned — linter via a plain child process, the other three via `@repo/agent-runner`'s `runSkillAgent`) and calling `ingestReport()`/writing status transitions, and (3) the WebSocket server that pushes those transitions to subscribed clients. Because execution and the WebSocket hub live in the _same_ process, "publish a status update" is a plain in-memory function call, not a cross-process bridge — the only cross-process problem to solve is step (1).

### How the orchestrator learns about a new scan: Postgres `LISTEN`/`NOTIFY`, not a direct call

`trigger-scan`'s action and `triggerScan.util.ts` (`packages/scan-ingestion`) are **unchanged** — still just `SELECT cqms.fn_create_run_with_scans(...)`. That DB function gains one line: `PERFORM pg_notify('cqms_scan_queued', run_id::text);` right after the insert. `apps/scan-orchestrator` holds one dedicated, non-pooled Postgres client (`LISTEN cqms_scan_queued;` — a persistent connection separate from `scan-ingestion`'s normal query pool, since `LISTEN` requires holding the connection open) and reacts the moment a notification arrives.

**`NOTIFY` is fire-and-forget, not durable** — if the orchestrator process isn't running (or is mid-restart) when `NOTIFY` fires, that specific notification is lost forever; it is not queued for a later listener. This is standard, well-known Postgres behavior for LISTEN/NOTIFY-based queues, not a novel gap, and it's handled the standard way: the orchestrator also runs a reconciliation query — `SELECT * FROM scans WHERE status = 'queued'` — once on startup and on a slow safety-net interval (e.g. every 30s) while running. `NOTIFY` gives near-zero latency in the common case; the reconciliation poll is the correctness backstop for the "orchestrator was down" case, not the primary mechanism.

Sequential execution (one queued scan at a time) is sufficient for v1 — genuinely low-traffic internal tool; a worker pool is a straightforward later enhancement, not a v1 requirement.

### The orchestrator's own server

Since it never serves CRUD or React Router traffic, it needs no Express and no `@react-router/express` adapter — a plain `node:http` server plus `ws`'s `WebSocketServer` attached to it (`new WebSocketServer({ server: httpServer, path: '/ws/runs' })`), optionally with one bare `GET /healthz` handler for liveness checks. Chose plain `ws` over `socket.io` here too: one narrow fan-out need (broadcast to clients watching one `run_id`), a small in-memory map covers it without socket.io's heavier abstraction.

**Hub**: `runStatusHub.util.ts`, now living in `apps/scan-orchestrator` (not `admin_system`, and still not `packages/scan-ingestion`, which stays framework-agnostic). An in-memory `Map<runId, Set<WebSocket>>`; on connect a client sends `{ type: 'subscribe', runId }` (validated as a real uuid, no further auth — internal tool), and `publish(runId, payload)` fans a JSON message out to every socket subscribed to that run, called immediately after each status transition the orchestrator itself just wrote.

**Client**: `useRunStatusSocket.hook.ts` (still in `admin_system`) opens a WebSocket inside a `useEffect` (see the guideline-fit note above) — now to the orchestrator's own origin/port (env-configured, e.g. `VITE_SCAN_ORCHESTRATOR_WS_URL`, not same-origin `/ws/runs` anymore, since it's a different process/port). On message, it calls `useRevalidator().revalidate()` rather than trusting the WS payload as UI state directly — **the socket is a cache-invalidation signal, not a data channel**; the loader stays the single source of truth for data shape. Includes basic reconnect-with-backoff on disconnect; no polling fallback in Phase 1 — accepted risk (a socket that can't reconnect needs a manual page refresh).

### What this buys, and what it costs

Eliminates the single trickiest item in the original design — reconciling one custom server entry with both `react-router dev`'s Vite-owned `http.Server` and a separate production Express server no longer applies, because `admin_system`'s server is never touched at all, in either mode. It also isolates blast radius in both directions: an HMR reload or an unrelated CRUD bug in `admin_system` can't drop a live WebSocket or kill an in-flight scan, and a bug in the scan-execution path can't take down project registration/editing. `ANTHROPIC_API_KEY` (§2.6) now belongs entirely to `apps/scan-orchestrator` — `admin_system` never needs it at all, a smaller credential footprint than the single-process design. The cost: two processes to run and monitor instead of one (`dev`/`start` need to launch both), and the client needs a second, explicitly-configured WS origin instead of same-origin `/ws/runs`.
...
| `trigger-scan.route.ts`  | —                                                                                                                                                                                                                           | Built with the new `Form` component (§2.10) — Zod-validates chosen scanners, inserts `runs`+`scans` rows (`queued`) — never spawns or awaits anything itself; `apps/scan-orchestrator` picks the run up via `LISTEN`/`NOTIFY` (§2.7) — redirects to run-detail — the app's first genuine domain-mutation action |
...
`@anthropic-ai/claude-agent-sdk` (agent-runner, used for 3 of 4 scanners — a dependency of `apps/scan-orchestrator`, not `admin_system`), `react-markdown` (packages/ui), `ws` + `@types/ws` (`apps/scan-orchestrator` only — plain `node:http`, no Express, no `@react-router/express`; `admin_system` keeps its stock `@react-router/serve`/`react-router dev` setup untouched, per §2.7's two-process split). The linter skill and the new `Form` component need no new runtime dependency. No UUID-generation library needed client/server-side — Postgres generates them; TypeScript types are just `string`.
...
2. `Form`'s scope is deliberately Phase-1-sized (`text`/`number`/`boolean`/`select`/`date`/`custom` leaf types) — richer types the reference project only aspired to (`tags`, `json`, `password-checker`) are registry additions later, not blockers now.
3. `LISTEN`/`NOTIFY`'s non-durability (§2.7) means the reconciliation poll in `apps/scan-orchestrator` is load-bearing, not decorative — it's the only thing that recovers a scan queued while the orchestrator was down. Budget for and test that path explicitly (kill the orchestrator, trigger a scan, restart it, confirm the scan still runs) rather than only testing the happy-path `NOTIFY` case.
4. `dev`/`start` now need to launch **two** processes (`admin_system` + `apps/scan-orchestrator`) — worth a single `pnpm --parallel --filter` (or equivalent) script rather than expecting two manually-opened terminals every time, same spirit as this repo's existing `dev`/`dev:fast` scripts that already run multiple apps together.

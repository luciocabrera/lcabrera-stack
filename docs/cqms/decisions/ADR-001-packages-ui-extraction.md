...
- Two apps now share one `GlobalSettingsContext`/`NotificationContext`/`ThemeContext` — a behavior change in `apps/react-router` (none — it's the same code, just relocated) but a real coupling going forward: a change to notification/settings/theme behavior for CQMS's sake now also affects `apps/react-router`, and vice versa.

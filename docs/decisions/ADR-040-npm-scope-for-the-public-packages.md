...
**Historical ADRs were not rewritten.** The mechanical rename initially edited 15 of them, including making the `packages/data-access` rename record claim a rename of `@lcabrera/api → @repo/data-access` that never happened. All were reverted. An ADR is a dated record of what was true when written, and this one is the pointer for anyone reading them later.

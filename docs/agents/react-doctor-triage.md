...
4. **Acting on it would contradict ADR-007.** Rule 2 of
   [ADR-007](../decisions/ADR-007-barrel-export-boundaries.md)
...
| [`useAddFilterSection.hook.ts`](../../packages/ui/src/components/Table/TableSettingsDrawer/FiltersSection/AddFilterSection/useAddFilterSection.hook.ts)                                                                                                                         | `columns.filter((col) => col.isFilterable !== false).map(…)` | Drawer-open only, and memoized by the React Compiler on `[columns, filters]` per [ADR-004](../decisions/ADR-004-react-compiler.md), so unrelated state changes do not re-run it. Trap: `isFilterable !== false` means **undefined ⇒ filterable**; a rewrite to `if (col.isFilterable)` silently drops every column that omits the flag, which is most of them.                                                                                                                                                                                                                                                                                |

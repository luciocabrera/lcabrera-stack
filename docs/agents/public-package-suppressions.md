...
practice. Several of the packages carry a local `rules` block, and every one
...
- **tsconfig strictness is not checked here.** Weakening `strict` or
  ...
  files are generated by `@repo/ts-configs` and belong to that generator. Every
  one is strict today; nothing yet fails if one stops being.

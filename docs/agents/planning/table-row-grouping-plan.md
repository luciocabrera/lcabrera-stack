...
| 5   | **Expansion resetting on revalidation reads as a bug**                                                                                                                                    | Path-keyed re-application survives sort changes (the common case) and most filter changes. Document it; the escape hatch is `sessionStorage`, which this repository has no reader for — taking it means building one, and ADR-061 prices that (#627) |

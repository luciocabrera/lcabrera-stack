...
const buildCreatePricingTab = () => ({
  fields: [buildPricingInputsGroup()],
  label: 'Pricing',
});

const buildEditPricingTab = () => ({
  fields: [
...
const buildCreateNotesTab = () => ({
  fields: [buildNotesGroup()],
  label: 'Notes & Audit',
});

const buildEditNotesTab = () => ({
  fields: [
    buildNotesGroup(),
    fieldGroup({
      collapsible: true,
...
const toOrderFormFields = (tabs: readonly OrderFormTab[]) =>
  [
    {
      tabs,
      type: 'tab',

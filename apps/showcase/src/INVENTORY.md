...
| `toOrderGroupLockedFilters` | `.server/toOrderGroupLockedFilters.util.ts` | This route's binding of `@lcabrera/server`'s `toGroupRestrictions`, as the table's `lockedFilters`: the catalogue lookup a truncated key's label needs, plus the refusal a route serving nothing but one group owes a reader when the token does not parse |

...
- Fallow full (`vp run fallow:full`): 25 above threshold · maintainability 93 (good) · 3204 analyzed
...
| ----- | ----- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| 1     | B-009 | WS-008: URL/filter serialization hotspots (`serializeFilter` CRAP=79.4, `fetchInitial` CRAP=63.6, `fetchMore` CRAP=88) | Reduce high cognitive/cyclomatic pressure in URL serialization and filter data path while preserving behavior | Targeted tests + full gate + `vp run fallow:full` |
| 2     | B-010 | Route loader complexity (`enterprise-orders.loader` CRAP=182, `wide-alltypes-150.loader` CRAP=132)                     | Extract formatter/badge helpers to pure functions, add coverage                                               | Targeted tests + full gate + `vp run fallow:full` |

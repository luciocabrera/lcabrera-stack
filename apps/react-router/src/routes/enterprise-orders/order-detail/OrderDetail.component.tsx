...
const Field = ({
  ...
  return (
    <div {...stylex.props(styles.field)}>
      ...
      <span {...stylex.props(styles.fieldValue, isEmpty && styles.emptyValue)}>{formatted}</span>
    </div>
  );
};
...
const NotesSection = ({ order }: { order: EnterpriseOrder }) => {
  ...
  const hasInternalNotes = order.internal_notes !== null && order.internal_notes !== "";
          ...
          <div {...stylex.props(styles.fieldGroup)}>
            ...
            {hasInternalNotes && (
              <div {...stylex.props(styles.field)}>
                ...
                <p {...stylex.props(styles.notesText)}>{order.internal_notes}</p>
              </div>
            )}
          </div>
        </CardBody>
    ...
    <div {...stylex.props(styles.container)}>
      <div {...stylex.props(styles.header)}>
        <div {...stylex.props(styles.headerLeft)}>
          <h1 {...stylex.props(styles.orderNumber)}>Order #{order.order_number}</h1>
          <span {...stylex.props(styles.badge, getStatusBadgeStyle(order.order_status))}>
            {order.order_status}
          </span>
        </div>
      ...
      <div {...stylex.props(styles.grid)}>
        <FieldSection fields={ORDER_INFO_FIELDS} order={order} title="Order Information" />
        <FieldSection fields={CUSTOMER_FIELDS} order={order} title="Customer" />
        <FieldSection fields={FINANCIAL_FIELDS} order={order} title="Financial" />
        <FieldSection fields={PAYMENT_FIELDS} order={order} title="Payment" />
        <FieldSection fields={PRODUCT_FIELDS} order={order} title="Product" />
        <FieldSection fields={FULFILLMENT_FIELDS} order={order} title="Fulfillment" />
        <FieldSection fields={SHIPPING_ADDRESS_FIELDS} order={order} title="Shipping Address" />
        <FieldSection fields={BILLING_ADDRESS_FIELDS} order={order} title="Billing Address" />

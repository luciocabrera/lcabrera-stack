...
now `@lcabrera/api`'s `config/config.constants.ts` (typed by
`@lcabrera/api/config/config.types`), and the filter-operator, global-settings and
pinning-preference option sets moved to `@lcabrera/ui/constants/` alongside the
...
of one feature or utility. And prefer `@lcabrera/ui` or `@lcabrera/utils` when the value
...
`APP_ID` namespaces the persisted cookie and storage keys written by `@lcabrera/ui`.

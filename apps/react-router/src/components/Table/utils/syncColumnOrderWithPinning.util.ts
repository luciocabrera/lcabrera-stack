...
export const syncColumnOrderWithPinning = ({
  ...
  const baseOrder = currentOrder.length > 0 ? currentOrder : columns.map((c) => c.key);
  ...
  if (columnPinning === "left") {
    ...
    const otherLeftPinnedCount = newPinning.left.filter((k) => k !== columnKey).length;
  ...
  const otherRightPinnedCount = newPinning.right.filter((k) => k !== columnKey).length;

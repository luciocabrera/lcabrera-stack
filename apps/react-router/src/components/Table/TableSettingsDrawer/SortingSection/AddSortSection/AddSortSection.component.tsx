...
export const AddSortSection = ({ onDropdownOpenChange }: AddSortSectionProps) => {
  ...
  const availableColumnOptions = columns
    .filter((col) => col.isSortable !== false && !sorting.some((s) => s.columnKey === col.key))
  ...
  return (
    <div {...stylex.props(styles.container)}>
      ...
      />
      {!isDropdownOpen && (
        <Button isDisabled={!selectedColumn} onClick={handleAddSort} size="sm" width="full">
          Add
        </Button>
      )}
    </div>

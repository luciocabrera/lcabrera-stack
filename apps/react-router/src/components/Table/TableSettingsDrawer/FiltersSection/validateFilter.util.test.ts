...
describe("validateFilter", () => {
  it("returns false for undefined", () => {
    const filterValue = void 0 as unknown as Parameters<typeof validateFilter>[0];
  ...
  it("returns false for date filter without value", () => {
    expect(validateFilter({ operator: "after", type: "date", value: "" })).toBe(false);
  });

  it("returns true for date filter with value", () => {
    expect(validateFilter({ operator: "after", type: "date", value: "2024-01-01" })).toBe(true);
  });
  ...
  it("returns false for select filter without values", () => {
    expect(validateFilter({ operator: "equals", type: "select", values: [] })).toBe(false);
  });
  ...
  it("returns true for number filter with value", () => {
    expect(validateFilter({ operator: "equals", type: "number", value: 0 })).toBe(true);
  });
  ...
  it("returns true for text equals without value", () => {
    expect(validateFilter({ operator: "equals", type: "text", value: "" })).toBe(true);
  });

  it("returns true for text notEquals without value", () => {
    expect(validateFilter({ operator: "notEquals", type: "text", value: "" })).toBe(true);
  });

  it("returns false for text contains with empty value", () => {
    expect(validateFilter({ operator: "contains", type: "text", value: "  " })).toBe(false);
  });

  it("returns true for text contains with value", () => {
    expect(validateFilter({ operator: "contains", type: "text", value: "hello" })).toBe(true);
  });

  it("returns false for multiSelect filter without values", () => {
    expect(validateFilter({ operator: "equals", type: "multiSelect", values: [] })).toBe(false);
  });

...
import { SelectAllOption } from "./SelectAllOption.component.tsx";
...
describe("SelectAllOption", () => {
  it('renders "Select All" when not all selected', () => {
    render(<SelectAllOption isAllSelected={false} isLoading={false} onSelectAll={() => void 0} />);
    expect(screen.getByText("Select All").textContent).toBe("Select All");
  });

  it('renders "Deselect All" when all selected', () => {
    render(<SelectAllOption isAllSelected isLoading={false} onSelectAll={() => void 0} />);
    expect(screen.getByText("Deselect All").textContent).toBe("Deselect All");
  });

  it("calls onSelectAll when checkbox is changed", () => {
    const onSelectAll = vi.fn();
    render(<SelectAllOption isAllSelected={false} isLoading={false} onSelectAll={onSelectAll} />);
  ...
  it("renders checkbox as disabled when isLoading is true", () => {
    render(<SelectAllOption isAllSelected={false} isLoading onSelectAll={() => void 0} />);
    const checkbox = screen.getByRole<HTMLInputElement>("checkbox");

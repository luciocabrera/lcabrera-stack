...
import { readFromCookie } from "./readFromCookie.util.ts";
...
describe("readFromCookie", () => {
  ...
  it("reads from provided cookieString instead of document", () => {
    ...
    expect(readFromCookie({ cookieString: "theme=light", key: "theme" })).toBe("light");
  });
  ...
  it("parses cookieString even without document", () => {
    vi.stubGlobal("document", undefined);
    expect(readFromCookie({ cookieString: "theme=dark; lang=en", key: "lang" })).toBe("en");
  });
});

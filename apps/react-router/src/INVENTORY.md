...
| `toOrderGroupHeading`    | `.server/toOrderGroupHeading.util.ts`    | The group-details modal's heading — one `Label: value` per key. Built server-side because the group token carries no labels: `encodeDrillGroup` drops the formatted string on purpose, and turning raw values back into text is `toGroupLabel`'s job in the Node-only package                                                                       |
| `resolveOrdersGroupRead` | `.server/resolveOrdersGroupRead.util.ts` | The read to run for a request that may name a group — scoped through `toOrderDrillRead` when it does. The group travels as a token rather than as filters, because a truncated key is a half-open range the filter vocabulary's inclusive `between` cannot express; an unreadable token is refused rather than falling through to the unscoped read |

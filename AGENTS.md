...
**Oxlint reads the root `vite.config.ts` `lint` block. A per-workspace `lint` block is never loaded.** Every workspace has one, built from a `@repo/vite-configs/*-lint` factory, and none of it reaches Oxlint — not `rules`, not `categories`, not `plugins`. The factories are not at fault: `createFrontendLintConfig()` returns a correct object with all its plugins and rules. Something below it drops the whole config. [#318](https://github.com/luciocabrera/vite-react-compiler/issues/318) chases the cause; until then, anything Oxlint must apply is declared at root.

The sharpest way to see it: put an unresolvable `jsPlugins` entry in a workspace's own lint config and `vp lint` there exits 0 without a word, while the same entry at root fails hard with "Failed to parse oxlint configuration file". A config that is read would object.

Two more behaviours are worth knowing, since both mask a broken setup as a clean one:

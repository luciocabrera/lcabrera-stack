...
├── data-access/         # Reusable server-side code — Postgres (src/db), filters, crypto, tokens
...
`packages/api` and `packages/data-access` are becoming public packages and are
held strictest: never baseline, scope, or inline-disable a finding in any of them.

`api` and `data-access` split on **runtime**, and the split is load-bearing, not
cosmetic. `@repo/api` is browser-safe — its tsconfig omits `node` types, so a
`process`/`fs` reach-in fails typecheck there. `@repo/data-access` is Node-only
(`pg`, `node:crypto`). They were one package until the cost showed up: `@repo/ui`

...
This repo does **not** follow that model yet: every workspace has a `lint` block built from a `@repo/vite-configs/*-lint` factory, and none of it is loaded. Put an unresolvable `jsPlugins` entry in one and `vp lint` there exits 0 without a word, while the same entry at root fails hard with "Failed to parse oxlint configuration file". [#318](https://github.com/luciocabrera/vite-react-compiler/issues/318) tracks the restructure. Until it lands, anything Oxlint must apply is declared at root.
...
Two more behaviours mask a broken setup as a clean one:

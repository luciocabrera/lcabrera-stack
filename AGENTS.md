...
**Oxlint's severity comes from the ROOT `vite.config.ts` `lint` block, and nowhere else.** The per-workspace `lint:` entries that point at `@repo/vite-configs/*-lint` do **not** reach Oxlint — proven by setting a rule to `error` in the shared config a workspace consumes and watching the violation still report as a warning, from the workspace and from the root, under `vp lint` and `vp check` alike (#318 tracks resolving those four dead configs). Two consequences. Any severity that is meant to bite has to be set in the root block. And **a warning fails nothing** — `vp lint .` and `vp check` both exit 0 while printing "Found 0 errors and 1 warning", so a category left at Oxlint's default `warn` is advisory, not enforced. That is why the root block now pins `categories: { correctness: 'error' }`: until it did, a `debugger` statement passed every gate. The other categories are **not** free and each needs its own decision — measured on 2026-07-22 at perf 30, suspicious 742, restriction 7270, pedantic 7738, style 12760. Verify any severity change with a deliberate violation before trusting a green run: a rule that is not loaded reports exactly the same clean pass as code that is correct.

...
Three findings are scoped off in `overrides`, each with its reason inline — all three are cases where Biome is wrong, not where the code is: `noThenProperty` (JSON Schema `if`/`then` keywords, already adjudicated for Oxlint in the same file), `useExhaustiveDependencies` (`eslint-plugin-react-hooks` already owns that rule and passes; two linters must not both arbitrate it), and `noAriaHiddenOnFocusable` on `SpacerRow` (Biome models `<tr>` as focusable role=row; this Table sets no `role=grid`/`tabIndex`, and every alternative either trips another rule or announces a blank spacer row).

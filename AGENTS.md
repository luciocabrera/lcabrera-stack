...
**Oxlint config layers: the root `vite.config.ts` `lint` block is a baseline, and each workspace adds its own on top through `@repo/vite-configs/*-lint`.** That per-workspace layer is real — a `rules` entry set only in the shared frontend config makes `vp lint` and `vp check` fail inside `packages/ui`. Three things about it are not obvious, and each cost a wrong conclusion to learn:

- **A warning fails nothing.** `vp lint .` and `vp check` both exit 0 while printing "Found 0 errors and 1 warning". So a category left at Oxlint's default `warn` is advisory, not enforced. `correctness` sat at `warn` in the root block (by omission) _and_ in `baseLintSharedConfig`, which meant a `debugger` statement passed every gate. The root block now pins `categories: { correctness: 'error' }`.

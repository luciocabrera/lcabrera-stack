...
- Do NOT do this:
  ...
  Yes — **exactly**.
  If the **URL stays correct**, then by definition **(1) is not being mutated at the URL level**. That narrows this _a lot_.

What’s happening is almost certainly:

> **Draft state is leaking into the _context layer_, not the URL layer.**
...
So the bug is **between (2) and (3)** — _not_ between (1) and (2).
...
Even if you _think_ you’re only modifying “local state”:
...
This is _by far_ the most common cause.
...
- boom 💥
- stack trace tells you _exactly_ where
...
console.log(
  ...
  Object.is(context.filters[columnId], previousContextRef.current?.[columnId]),
);
```
...
## What _should_ be happening instead

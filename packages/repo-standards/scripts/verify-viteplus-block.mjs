 ...
 *
 * Usage: repo-verify-viteplus-block [--write]
...
const AGENT_DOC = readGates(REPO_ROOT).vitePlusBlock.agentDoc;
const TARGET = join(REPO_ROOT, AGENT_DOC);
...
const main = () => {
  ...
  if (region.kind === 'unpaired') {
    process.stderr.write(
      `${AGENT_DOC} has an unpaired Vite+ marker — ${START_MARKER} and ${END_MARKER} must both be present, in that order.\n`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (write) {
    ...
    process.stdout.write(
      `Repaired ${AGENT_DOC}: emptied the Vite+ managed region (${offending.length} rendered line(s) removed).\n`,
    );
    return;
  }

  process.stderr.write(
    `The Vite+ managed region in ${AGENT_DOC} is not empty — a \`vp\` run refilled it with\n` +

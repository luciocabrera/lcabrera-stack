...
import { commandTask } from './doc-registers.mjs';
...
const main = () => {
  ...
  const { ciCommands, planning, planningDir, requirements } = registers;
  const checked = planning.filter((entry) =>
    carriesPlanningBlock(entry, planningDir),
  );
  console.log(

...
import { readGates, readRegisters } from './config.mjs';
...
import { readHarnessInventory } from './usage-inventory.mjs';
import { renderReport } from './usage-render.mjs';
import { repositoryWorkingTrees } from './usage-scope.mjs';
import {
  countsFor,
  earliestDay,
  mergeObserved,
...
import { readTranscriptUsage, transcriptsRoot } from './usage-transcripts.mjs';
import { dayOf, shiftDay, windowOf } from './usage-window.mjs';

const REPO_ROOT = resolveHostRoot({
  moduleDirectory: dirname(fileURLToPath(import.meta.url)),
});
const REGISTER_DIRS = readRegisters(REPO_ROOT);
const DEFAULT_OUT_DIR = readGates(REPO_ROOT).usageReport.outDir;
...
const REGISTERS = [
  {
    detail: 'per-file',
    directory: REGISTER_DIRS.requirementsDir,
  ...
  {
    detail: 'summary',
    directory: REGISTER_DIRS.coordinationTasksDir,
...
const main = () => {
  ...
  const outDir = resolve(REPO_ROOT, args.out ?? DEFAULT_OUT_DIR);

...
const TREES = [
  ...
  { inventory: 'apps/showcase/src/INVENTORY.md', root: 'apps/showcase/src' },
];

describe('treeFor', () => {
  it('matches a file under one of the three governed trees', () => {
    expect(treeFor('packages/ui/src/hooks/useTheme.hook.ts', TREES)?.root).toBe(
      'packages/ui/src',
    );
    expect(
      treeFor('packages/server/src/db/get-pool.util.ts', TREES)?.root,
    ).toBe('packages/server/src');
    expect(
      treeFor('apps/showcase/src/auth/authMiddleware.ts', TREES)?.root,
    ).toBe('apps/showcase/src');
  });

  it('matches nothing for a file outside every tree', () => {
    expect(
      treeFor('packages/api/src/fetchAndValidate.util.ts', TREES),
    ).toBeUndefined();
    expect(treeFor('scripts/verify-inventory.mjs', TREES)).toBeUndefined();
  });
});
...
describe('utilFileEntries', () => {
  it('keeps only *.util.ts / *.util.tsx files inside a governed tree', () => {
    ...
    expect(utilFileEntries(paths, TREES).map((entry) => entry.file)).toEqual([
      'packages/ui/src/utils/theme/theme.util.ts',
      'packages/ui/src/components/Button/utils/getButtonElement.util.tsx',
    ]);
  });

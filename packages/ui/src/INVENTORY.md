...
| `TableActionButton`                  | `components/Table/TableActionButton/`                  | Private delegate — icon-only vertical-dots trigger button for row actions popovers in the table actions column                                                                                                                                                                                                                                                                                      |
...
| `TableRowActionsMenu`                | `components/Table/TableRowActionsMenu/`                | Private delegate — Popover API row menu that renders enabled CRUD actions (view/edit/delete) and appends custom action-column content                                                                                                                                                                                                                                                               |
...
| `PathField`                          | `components/Form/fields/PathField/`                    | Private delegate — text input + Browse… button for `path`; opens an inline `PathBrowserModal` list panel                                                                                                                                                                                                                                                                                            |
| `PathBrowserModal`                   | `components/Form/fields/PathField/PathBrowserModal/`   | Private delegate — inline directory drill-down list panel over `@repo/ui/routing/browseDirectory.loader` via `useFetcher().load`                                                                                                                                                                                                                                                                    |
...
| `hydrateTableColumnsState`                 | `components/Table/utils/hydrateTableColumnsState.util.ts`                 | Rehydrates a loader-seeded, serializable columns state with full client-side column definitions; forces `actions` pinned right             |
...
| `validateTableCrudConfig`                  | `components/Table/utils/validateTableCrudConfig.util.ts`                  | Validates CRUD invariants (≥ 1 operation enabled; read/update/delete require an `isPrimaryKey` column; delete requires `deleteActionPath`) |

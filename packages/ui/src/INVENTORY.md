...
| `Form`                               | `components/Form/`                                     | Declarative `fields`-driven form (ADR-005) — `group`/`row`/`tab` recursion, registry-dispatched leaf types, RR7 native `<Form>`/`useNavigation` by default, `create`/`edit`/`view` modes with dirty-check-gated edit submission |

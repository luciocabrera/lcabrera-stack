...
| `resolveCrudRowId`                         | `components/Table/utils/resolveCrudRowId.util.ts`                         | Resolves row ids for table CRUD links/actions from a configured key accessor or resolver function         |
...
| `validateTableCrudConfig`                  | `components/Table/utils/validateTableCrudConfig.util.ts`                  | Validates table CRUD config invariants (at least one operation enabled; delete requires deleteActionPath) |

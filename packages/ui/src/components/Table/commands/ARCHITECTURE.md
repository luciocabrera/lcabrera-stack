...
| `grouping/groupingCommands.ts`        | constants | `GROUP_BY_COLUMN_COMMAND`, `REMOVE_GROUP_KEY_COMMAND`, `CLEAR_GROUPING_COMMAND`, `EXPAND_ALL_GROUPS_COMMAND`, `COLLAPSE_ALL_GROUPS_COMMAND`, `EXPAND_GROUP_LEVEL_COMMAND`, `COLLAPSE_GROUP_LEVEL_COMMAND`, `AGGREGATE_COMMANDS` (a `Record` closed over `TableAggregateFn`), `CLEAR_COLUMN_AGGREGATE_COMMAND` |

...
/**
 * Split from `TableGroupKeyLink` for that reason alone: a table whose route declares no
 * `groupDetailsPath` never mounts this, so a grouped table still renders outside a router
 * exactly as it did before the affordance existed.
 * **It is not a tab stop, and that is the same rule the chevron follows.** ADR-062 gives
 * the grid a roving tab stop addressed by row key plus column key, so exactly one element
 * in the body is tabbable; an anchor that took focus would be a second stop inside a cell
 * that already owns one, and tabbing through a grouped body would alternate between the
 * two.
 */

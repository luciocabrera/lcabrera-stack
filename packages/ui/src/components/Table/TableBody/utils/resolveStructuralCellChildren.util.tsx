...
type ResolveStructuralCellChildrenArgs = {
   ...
   * `hasTableStructuralMarker`. Separate from the narrowed summary above
   * because that cannot distinguish a malformed marker from an absent one.
   */
 ...
 *
 * 1. **A group row's own cells**, asked of the row and never of the grouping
 *    configuration, so a group row and a detail row can arrive in one result.
 * 2. **A row that claims to be structural but did not narrow**, blanked whole.
 *    It is a malformed group row, never a data row, and saying so here is what
 *    keeps a bad marker inside the branch that knows what to do with it.

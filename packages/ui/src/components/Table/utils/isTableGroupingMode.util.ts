 ...
 * Tested against `TABLE_GROUPING_MODE_LABELS`, a map closed over `TableGroupingMode`, so
 * the guard is total by construction — the same shape `isTableAggregateFn` uses, and for
 * the same reason: a member added to the union forces a label entry and is recognised here
 * the same day.
 */

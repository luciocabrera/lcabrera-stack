...
const isSettingsTabRole = (value: string): value is TableSettingsTabRole =>
  TABLE_SETTINGS_TAB_ROLES.includes(value as TableSettingsTabRole);

export const resolveSettingsTabOrder = (
  order: readonly string[] | undefined,
) => {
  const declared = [...new Set(order)].filter(isSettingsTabRole);

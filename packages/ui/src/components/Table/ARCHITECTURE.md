...
mapper. The column-view selectors derive the emitted list from the current
group rows, so a loaded response and its painted columns stay in one read
path. `withGroupedColumnScope` then drops every

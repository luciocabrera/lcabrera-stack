...
- `AppShell` — no props. Theme state comes from `useTheme()` (reads the `ThemeProvider` above it, supplied by `AppProviders`); routed content comes from React Router's own route tree via `<Outlet />`.
...
- `AppNavigation` — left/side navigation, receives the theme toggle.

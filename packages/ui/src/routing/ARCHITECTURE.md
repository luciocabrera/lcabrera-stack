...
| `startInjectedResolver.util.ts`           | Starts one injected resolver and returns a promise whatever it does, so a resolver entitled to answer synchronously cannot throw past the `Promise.all` that owns the others and strand one (ADR-094)                                                                                                                                                                                                                                |
| `resolveTableCapabilityMeta.util.ts`      | Resolves the request-shaping capabilities from the route's `meta` alone (ADR-063). `metaState` also draws on the client-controlled UI-flags cookie, so this is spread last to keep absent meaning off whatever that cookie carries                                                                                                                                                                                                   |
...
| `sanitizeLayoutByColumns.util.ts`      | Drops cookie and URL column keys that are not in the route's column set; a malformed slice becomes empty        |

...
| `selectFilterOptions`           | `db/select-filter-options.util.ts`            | Filter-dropdown read over `selectDistinctRows`: one column's distinct, non-empty (empty dropped only for `text`), ordered values → `{ values, hasMore }`. Pair it with `filters/resolveFilterOptionsSource` when the schema/table arrive on a request                                                                                 |

...
| `selectColumnAxisValues`        | `db/select-column-axis-values.util.ts`        | Private. Capped `SELECT DISTINCT` of one column, same filters as the grouped read, refusing past `maxDistinct` by name. Keeps NULLs. Used by `selectGroupedRows`                                                                                                                                                                                                                                                                                 |
...
`assertGroupSort`, `resolveAggregateAlias`, `expandColumnAxisAggregates`,
`toColumnAxisAlias`, `buildAggregateProjection`, `buildGroupingSetsClause`,

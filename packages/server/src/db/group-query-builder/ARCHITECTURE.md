...
| `assert-column-axis.util.ts`              | The column-axis gates: caller `maxDistinct` (`assertColumnAxisMaxDistinct`, also used before DISTINCT), not also a row key, catalogue-legal, and the Postgres heap-attribute ceiling      |

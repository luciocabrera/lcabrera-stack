...
Typed translation of `pg` rejections. One job: **a driver error never leaves this
...
| `errors.types.ts`                      | **Public.** `PgErrorFields`                                                  |
| `read-pg-error-fields.util.ts`         | Private. Lifts those fields off an unknown rejection                         |
| `errors.constants.ts`                  | Private. The two SQLSTATEs `mapDbError` names                                |

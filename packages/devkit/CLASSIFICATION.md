...
| `grok-review.yml`           | —       | —      | **repo-specific** | —          | Runs a catalog reviewer under `XAI_API_KEY` and a second reviewer app's key. Same reason as `claude-review.yml`: credentials do not travel, and a job that skipped every step without them would report success to a consumer who has nothing working.                                                                                                                          |
...
| `shipped-ranges:verify`     | —       | —      | **repo-specific** | —          | Reads the ranges `packages/devkit` ships against the versions **this** repository publishes. A consumer publishes neither side.              |

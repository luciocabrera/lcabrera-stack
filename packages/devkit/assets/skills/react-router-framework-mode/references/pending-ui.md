...
For mutation pattern selection (`Form` vs `useFetcher`), see [actions.md](./actions.md#choosing-the-right-pattern).
...
const RatingStars = ({ itemId, currentRating }) => {
  const fetcher = useFetcher();
...
export const MyComponent = ({ loaderData }: Route.ComponentProps) => {
  return (
    <div>

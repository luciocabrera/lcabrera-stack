...
| `vp run inventory:verify`    | check every `*.util.ts`/`*.util.tsx` export is named (in backticks) somewhere in its tree's own `INVENTORY.md` (`--write` regenerates `scripts/inventory-drift-baseline.json`, reviewed as a JSON diff) |

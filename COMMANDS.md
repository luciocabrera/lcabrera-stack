...
| `vp run shipped-ranges:verify` | check every dependency range in the workspace assets `packages/devkit` ships admits the version this repository publishes for that package, and the minor after it; a shipped file naming one of those packages that yields no declaration of it fails too, per file, so a reader gone quiet on one source cannot pass behind another that still answers |

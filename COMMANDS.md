...
| `vp run shipped-ranges:verify` | check every dependency range in the workspace assets `packages/devkit` ships admits the version this repository publishes for that package, and the minor after it; a shipped file naming one of those packages that yields no declaration of it fails too, per file, as does a run that read no manifest or no workspace catalog at all — so neither a reader gone quiet nor a narrowed walk can pass behind a source that still answers. Only `catalog:` and `workspace:` are exempt from judgement; an `npm:`, `https:` or `file:` specifier pins an artifact and is refused |

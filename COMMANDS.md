...
| `vp run shipped-ranges:verify` | check every dependency range in the workspace assets `packages/devkit` ships admits the version this repository publishes for that package, and the minor after it; it scans every shipped data file for those package names and reads declarations from the manifests and workspace catalogs among them, so a file naming one and yielding no declaration of it fails, per file, whether its reader went quiet or it left the reader's set entirely. Only `catalog:` and `workspace:` are exempt from judgement; an `npm:`, `https:` or `file:` specifier pins an artifact and is refused |
...
| `vp run devkit:sync`           | materialise the shipped files into this repository from [`packages/devkit`](packages/devkit/CLASSIFICATION.md) — a locally modified file is reported and kept, never overwritten; `-- --profile repo` also places the workflows, hooks, templates and `COMMANDS.md`                                                                                                                                                                                                                                                                                                                        |
...
| `vp run harness:verify`        | check every skill, path rule and subagent conforms — parseable frontmatter, the fields its loader reads, every path it names resolving, and a description that says when to use it; the roster is read from `.github/skills`, `.claude/rules` and `.claude/agents` on each run                                                                                                                                                                                                                                                                                                             |

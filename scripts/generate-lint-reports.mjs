/**
 * Generates machine-readable JSON reports for BOTH linters into the
 ...
 *
 * ESLint runs in CHECK mode (no --fix) — generating a report never mutates
 * sources. Lint findings do not fail the script (they ARE the report); only
 * fatal tool errors (bad config, crash) do.
 *
 * Usage (from the repo root):
 *   vp run lint:report                          # both tools
...
/** Returns the validated --only=<tool> value, or undefined for "both". */
const parseOnlyArgument = () => {
  ...
  if (!['eslint', 'oxlint'].includes(only)) {
    throw new TypeError(
      `--only expects "eslint" or "oxlint", received "${only}"`,
    );
  }
  return only;
};

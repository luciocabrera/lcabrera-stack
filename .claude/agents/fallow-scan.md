...
2. Default scope is the entire monorepo — fallow is configured once at the repo root (`.fallowrc.json`) and auto-detects all pnpm workspaces. If the caller asks for a narrower scope, pass a workspace glob to the runner (e.g. `bash .github/skills/fallow-code-checker/scripts/run-fallow.sh 'apps/react-router'`).

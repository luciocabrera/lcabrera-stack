...
- **Functions**: small (CHK.FUNC.LONG), do one thing, one level of abstraction (CC.G34), ≤3 args (CC.F1), no boolean flag args (CC.F3), no output args (CC.F2), no side effects in a pure home (CHK.FUNC.SIDE-EFFECT).
- **Names**: reveal intent (CC.N1), unambiguous (CC.N4), longer for longer scopes (CC.N5), no type encodings (CC.N6), describe side effects (CC.N7).
- **Comments**: explain _why_ not _what_; delete obsolete (CC.C2), redundant (CC.C3), commented-out (CC.C5).
- **General**: duplication is the worst smell (CC.G5); vertical separation (CC.G10); polymorphism over switch (CC.G23); encapsulate conditionals (CC.G28); encapsulate boundary-index math (CC.G33); hidden temporal coupling (CC.G31); config at high levels (CC.G35); avoid Law-of-Demeter violations (CC.G36); replace magic numbers with named constants (CC.G25).
- **Tests**: F.I.R.S.T. — Fast (CC.T9), Independent (CHK.TEST.DEPENDENT), Repeatable (CHK.TEST.NONDETERMINISTIC), Self-validating, Timely; test boundary conditions (CC.T5).
...
- **CC.G33** Encapsulate Boundary Conditions (`length - 1` / next-index / last-row math scattered; not CC.G28, which names a boolean predicate)
